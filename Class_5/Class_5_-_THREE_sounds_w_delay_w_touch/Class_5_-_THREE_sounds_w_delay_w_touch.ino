...
AudioSynthWaveform       waveform1;      //xy=136.0056915283203,191.99999260902405
AudioSynthNoiseWhite     noise1;         //xy=151.0056915283203,362.99999260902405
AudioEffectEnvelope      envelope1;      //xy=252.0056915283203,240.99999260902405
AudioEffectEnvelope      envelope2;      //xy=361.0056915283203,370.99999260902405
AudioSynthSimpleDrum     drum1;          //xy=392.0056915283203,179.99999260902405
AudioFilterStateVariable filter1;        //xy=402.0056915283203,260.99999260902405
AudioEffectDelay         delay1;         //xy=565.0056915283203,414.0056838989258
AudioMixer4              mixer1;         //xy=575.0056915283203,260.99999260902405
AudioOutputAnalog        dac1;           //xy=758.0056915283203,226.99999260902405
...
void setup() {
  ...
  waveform1.begin(WAVEFORM_SAWTOOTH); //begin(waveshape) //https://www.pjrc.com/teensy/gui/?info=AudioSynthWaveform
  ...
  //envelope2.release(50); //how long will the note fade out in millis after button is released
  envelope2.attack(1); //how long will the note fade out in millis after button is released
  envelope2.decay(15); //how long will the note fade out in millis after button is released
  envelope2.sustain(0); //how long will the note fade out in millis after button is released
  delay1.delay(0, 100);
}
...
void loop() {
  ...
  touch0 = touchRead(23);
  int threshold0 = 3000;
  if (prev_touch0 < threshold0 && touch0 > threshold0) {
    int dl = (touch0 - 3000) / 4.0;
    drum1.length(dl); //Length of sound in milliseconds
    Serial.println(dl);
    drum1.noteOn();
    ...
    for ( int x_count = 0; x_count < 8; x_count++) {
      for ( int y_count = 0; y_count < 8; y_count++) {
        ...
        if (middle_button.read() == 0) {
          if (bitmap[xy_count] > 0 && y_count >= 4) {
            //set_pixel_HSV(led to change, hue,saturation,value aka brightness)
        ...
        if (right_button.read() == 0) {
          if (bitmap[xy_count] == 0) {
            //set_pixel_HSV(led to change, hue,saturation,value aka brightness)
            float r1 = random(100) / 400.0;
            float color1 = random(100) / 200.0;
            set_pixel_HSV(xy_count, color1 , 1 - r1 , .5 - r1);
          }
        }
      }
    }
