...
void loop() {
  ...
  if (current_time - prev_time[2] > rate3) {
  ...
  if (current_time - prev_time[0] > rate1) {
    ...
    x_sel = map(analogRead(top_left_pot_pin), 0, 4095, 0, 7);
    y_sel = 7 - map(analogRead(top_right_pot_pin), 0, 4095, 0, 7);
    ...
    for ( int x_count = 0; x_count < 8; x_count++) {
      for ( int y_count = 0; y_count < 8; y_count++) {
        ...
        if (y_count == y_sel) { //&& means and. A single & is a whole other thing
          //set_pixel_HSV(led to change, hue,saturation,value aka brightness)
          set_pixel_HSV(xy_count, colors[1] , 1, 1);
        }

        if (x_count == y_count) { //&& means and. A single & is a whole other thing
          //set_pixel_HSV(led to change, hue,saturation,value aka brightness)
          set_pixel_HSV(xy_count, colors[0], 1, 1);
        }

        if (x_count == random(0, 10)) { //&& means and. A single & is a whole other thing
          //set_pixel_HSV(led to change, hue,saturation,value aka brightness)
          set_pixel_HSV(xy_count, colors[random(0, 4)], 1, 0);
        }
    ...
    int h;
    for (int j = 0; j < 4; j++) {
      set_pixel_HSV(h, colors[random(0, 4)], 1, 1);
      h += 10;
    }
