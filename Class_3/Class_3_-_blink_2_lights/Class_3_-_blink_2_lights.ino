  ...
  if (current_time - prev_time[0] > rate1) {
    ...
    for ( int x_count = 0; x_count < 8; x_count++) {
      ...
      for ( int y_count = 0; y_count < 8; y_count++) {
        ...
        if (x_count == 4 && y_count == 6) { //&& means and. A single & is a whole other thing
          //set_pixel_HSV(led to change, hue,saturation,value aka brightness)
          set_pixel_HSV(xy_count, .8 , 1, led1_state);
        }

        if (x_count == 6 && y_count == 2) { //&& means and. A single & is a whole other thing
          //set_pixel_HSV(led to change, hue,saturation,value aka brightness)
          set_pixel_HSV(xy_count, .2 , 1, led2_state);
        }
      }
