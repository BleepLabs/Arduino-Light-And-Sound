...
//Two slashes means "comment". // Anything written after the "//" is ignored 
// by the computer as it uploads the code to the Teensy
/*
   This has the same effect
*/

// "int" aka integer describes a data type which allows the variable 
...
//This next part of the code runs just once after the Teensy resets.
...
void loop() {
  ...
  // contained in the following curly brackets?
  // If the current time minus the last time is bigger that the interval then 
  ...
  if (current_time - previous_time > interval1) {
    //remember what time it is now so we can check how long it's been next loop
    previous_time = current_time;  

    // if the LED is off turn it on and vice versa:
    if (led1_state == 0) { // "==" means compare if these two values are the same
