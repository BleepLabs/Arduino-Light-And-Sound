...
void loop() {
  ...
  if (current_time - prev[0] > 30) {
    ...
    for ( int i = 0; i < num_of_leds; i++) { // this block of code will repeat 64 times with i going from 0-63
      ...
      set_hue = i / 64.0;
      set_pixel_HSV(led_sel, set_hue, 1.0, 0);
    }

    int led_off_sel = map(analogRead(A0), 0, 4095, 0, num_of_leds - 1);
    set_pixel_HSV(led_off_sel, 0, .5, led1_state);

    set_pixel_HSV(random(0, 64), .5, 1.0, 1.0);
