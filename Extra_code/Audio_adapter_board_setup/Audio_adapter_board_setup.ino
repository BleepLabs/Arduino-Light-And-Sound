//This code shows how to setup the audio adapter board
...
void setup() {
  ...
  //the control object sgtl5000 is necessary to use the adapter. 
  // see all of it's functions here https://www.pjrc.com/teensy/gui/?info=AudioControlSGTL5000
  // it's placed in the tool but not connected to anything
  sgtl5000_1.enable(); //Turn the adapter board on
  
  sgtl5000_1.inputSelect(AUDIO_INPUT_LINEIN); //Tell it what input we want to use. 
  sgtl5000_1.lineInLevel(13); //The volume of the line input. 0-15, the bigger the value the louder the input

  //to use the mic use these instead. Only one can be used, lin or mic 
  //sgtl5000_1.inputSelect(AUDIO_INPUT_MIC); 
  //sgtl5000_1.micGain(20); //0-63dB
  

  //Output volume. Goes from 0.0 to 1.0 but a fully "1.0" signal will clip over .8 or so.
  // For headphones it's pretty loud at .4
  // There are lots of places we can change the final volume level. This one you set once and leave alone.
  sgtl5000_1.volume(0.25);
  
  //the hadpone and line outs will always put out the same audio but can have thier audio adjusted
  // in general it's better to use the line when connecting to mixers, amps, etc, but it's not a huge deal
  sgtl5000_1.lineOutLevel(29); //13-31, the samller the number the louder the output siganl 
